...
- [tile_families.py](tile_families.py) — generic tile-family loader and query model: family / cluster / tile catalogs, alias resolution, address parsing, construction loading and validation. Also exposes `TileLibraryUnit`, a self-contained runtime-facing compatibility view that lets scene composition depend on a narrower library surface than the raw source-shaped `TileFamily`.
