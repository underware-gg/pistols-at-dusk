...
- **Adjacency.** Every internal edge between two filled fixed-construction cells must fit by derived seam profile. The validator visits canonical east/south contacts once, compares the facing sides under the default seam-match policy, and fails the family load when the seams do not fit. A fixed construction may declare a `seam_overrides` entry for a specific contact (`cell: {x, y}`, `side: "east" | "south"`, and a non-empty `reason`) when the mismatch is an intentional authored exception; an override whose seam later fits is rejected as stale. `parametric_run` uses the same seam-profile policy for its start/repeat/end contacts. `parametric_frame` corner/edge seam verification remains deferred because flipped and fat-corner orientation needs its own treatment. `connects_on` remains tile metadata, but it is no longer trusted as construction-adjacency validation input.
