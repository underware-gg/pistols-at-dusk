...
- **Adjacency.** Every internal edge between two filled fixed-construction cells must fit by derived seam profile. The validator visits canonical east/south contacts once, compares the facing sides under the default seam-match policy, and fails the family load when the seams do not fit. A fixed construction may declare a `seam_overrides` entry for a specific contact (`cell: {x, y}`, `side: "east" | "south"`, and a non-empty `reason`) when the mismatch is an intentional authored exception; an override whose seam later fits is rejected as stale. `parametric_run` uses the same seam-profile policy for its start/repeat/end contacts. `parametric_frame` remains structurally validated only in this sequence; frame seam verification is promoted to the role-aware construction grammar described by [ADR 0010](decisions/0010-runtime-flippability-and-role-aware-frame-seams.md), because corner/cap joins and repeating connectors are different role-defined relations rather than one uniform silhouette gate. `connects_on` remains tile metadata, but it is no longer trusted as construction-adjacency validation input.
